...
MySceneGraph.prototype.parseAnimations = function(nodesAnimation) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if (animationType == "circular"){
            var center = [];
            center[0]= this.reader.getFloat(children[i], 'centerx');
            center[1]= this.reader.getFloat(children[i], 'centery');
            center[2]= this.reader.getFloat(children[i], 'centerz');
            ...
            this.animations[animationID] = new CircularAnimation(this.scene,animationID, center, radius, startAng, rotAng, animationSpeed);
            ...
            for(var k=0; k < controlPoints.length; k++) {
                if(controlPoints[k].nodeName === 'controlpoint') {
                    ...
                    if (xx == null ) {
                        this.onXMLMinorError("unable to parse xx component of controlPoint");
                    } else {
                        cpPoint.push(xx);
                    }
                    var yy = this.reader.getFloat(controlPoints[k], 'yy');
                    if (yy == null ) {
                        this.onXMLMinorError("unable to parse yy component of controlPoint" );
                    } else {
                        cpPoint.push(yy);
                    }
                    var zz = this.reader.getFloat(controlPoints[k], 'zz');
                    if (zz == null ) {
                        this.onXMLMinorError("unable to parse zz component of controlPoint" );
                    } else {
                        cpPoint.push(zz);
                    }

                    animationWithControlPoints.push(cpPoint);
                ...
                for (var j = 0; j < animationDescen.length; j++) {
                    if (animationDescen[j].nodeName = "ANIMATIONREF")
                    {
                        ...
                        this.onXMLMinorError("unable to parse descendant id animation");
                            ...
                            this.nodes[nodeID].addAnimation(this.animations[curId]);
...
MySceneGraph.prototype.displayScene = function() {
    ...
    this.nodes[this.idRoot].display(this.defaultMaterialID, this.nodes[this.idRoot].textureID, 1, 1, false, false);
